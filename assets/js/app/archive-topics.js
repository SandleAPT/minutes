(function () {
  ...
  var DEFS = [
    ...
    { key: "도서관", kw: ["도서관", "사서"] },
    { key: "커뮤니티센터", kw: ["커뮤니티", "노크타운", "위탁관리업체", "위탁업체"] },
    { key: "주차", kw: ["주차", "차단기", "반사경"] },
    { key: "수광선", kw: ["수광선", "비상대책", "철도"] },
    { key: "LH·관리이관", kw: ["LH", "관리이관", "관리 이관"] },
    { key: "회계·결산", kw: ["결산", "이익잉여", "회계감사", "외부회계", "위탁관리수수료"] },
    { key: "관리비", kw: ["관리비", "부과 명세", "부과 내역", "부과내역"] },
    { key: "잡수입·예산", kw: ["잡수입", "격려금", "휴가비", "예산"] },
    { key: "관리규약", kw: ["관리규약", "규약 개정"] },
    { key: "선거·임원", kw: ["보궐선거", "임원", "동별 대표", "동별대표", "선출", "선거관리위원회"] },
    { key: "장기수선", kw: ["장기수선", "충당금", "재도장", "방수", "균열", "보도블럭", "아스팔트"] },
    { key: "승강기", kw: ["승강기", "엘리베이터"] },
    { key: "화재·소방", kw: ["화재", "소화기", "스프링클러", "소방"] },
    { key: "조경·환경", kw: ["철쭉", "회양목", "조경", "고사목", "금연", "불법소각", "묘목", "식재"] },
    { key: "지원사업", kw: ["지원사업", "공모사업", "성남시", "지원 사업", "공동체활성화", "공동체 활성화"] },
    { key: "저수조·청소", kw: ["저수조", "청소용역", "청소 용역", "청소미화", "미화원", "미화 인력"] },
    { key: "교통·버스", kw: ["버스", "회차지", "셔틀", "불법차량"] },
    { key: "전기·설비", kw: ["전기안전", "직무고시", "기계설비", "승압", "변압기", "수변전"] }
  ];
  ...
  function tagsOf(a) {
    if (Array.isArray(a.tags) && a.tags.length) return a.tags.map(function (t) { return String(t).trim(); }).filter(Boolean);
    if (a.category && String(a.category).trim()) return [String(a.category).trim()];
    return autoTags(a);
  }
