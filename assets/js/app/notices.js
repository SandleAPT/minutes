...
//   v355부터 공고·안내·절차 점검과 같은 열람 비밀번호 확인 뒤에만 불러온다 — 카페 공개 게시본이지만
//   한자리에 모아 두면 성격이 달라진다는 판단(다른 세션에서 결정, 2026-09-03).
...
var Notices=(function(){
  ...
  function lockHtml(){
    return '<div class="nt-lock"><div class="nt-lock-ic">🔒</div><b>비밀번호 확인이 필요한 기록입니다</b> <span class="small">(열람용 또는 수정용 비밀번호)</span>'+
      '<p>계약·기준문서, 공고·안내 보관함, 절차 점검 기록은 열람 비밀번호를 입력한 기기에서만 볼 수 있습니다.</p>'+
  ...
  function impactText(i){
    var s=String(i.summary||'').trim(), first=(s.match(/^.*?[.!?](?:\s|$)/)||[])[0]||s;
    return [i.statusDetail||'',first].filter(Boolean).join(' · ');
  }
  function detailGroup(title,arr,kind){
    if(!arr||!arr.length) return '';
    return '<details class="pc-detail-group '+(kind||'')+'"><summary><span>'+esc(title)+'</span><small>'+arr.length+'건</small><span class="pc-detail-arrow" aria-hidden="true">⌄</span></summary>'+
      '<div class="pc-detail-group-body"><ul class="nt-facts">'+arr.map(function(x){return '<li>'+esc(x)+'</li>';}).join('')+'</ul></div></details>';
  }
  function relatedGroup(rel){
    if(!rel||!rel.length) return '';
    return '<details class="pc-detail-group pc-related"><summary><span>관련 기록</span><small>'+rel.length+'건</small><span class="pc-detail-arrow" aria-hidden="true">⌄</span></summary>'+
      '<div class="pc-detail-group-body">'+relChips(rel)+'</div></details>';
  }
  function cardDetail(i){
    var sections=detailGroup('확인된 사실',i.facts,'pc-facts')+detailGroup('적용 기준',i.rules,'pc-rules')+detailGroup('확인자료',i.evidence,'pc-evidence')+relatedGroup(i.related);
    return sections?'<div class="pc-detail-list"><div class="pc-detail-list-title">근거를 종류별로 보기</div>'+sections+'</div>':'';
  }
  function investigationCard(i){
    ...
    return '<details class="rl-art pc-card pc-'+tone+'" id="chk-'+esc(i.id||'')+'">'+
      '<summary class="pc-card-summary">'+
        '<div class="pc-card-main">'+
          ...
          '<p class="pc-card-impact">'+esc(impactText(i))+'</p>'+
          '</div>'+
        '<span class="pc-card-arrow" aria-hidden="true">⌄</span>'+
      '</summary>'+
      '<div class="pc-card-body">'+
        '<section class="pc-story"><h4>한눈에 보기</h4><p>'+esc(i.summary||'세부 확인 내용이 정리 중입니다.')+'</p></section>'+
  ...
  function flowHtml(flow){
    return flow?'<p class="pc-flow"><b>흐름</b> '+esc(flow)+'</p>':'';
  }
  ...
  function investigationsHtml(){
    ...
    h+='<div class="nt-note pc-guide"><p>'+esc((st.investigations&&st.investigations.note)||'')+'</p><p class="pc-guide-tip">닫힌 카드의 핵심 한 줄 → 펼친 카드의 요약·확인할 것 → 종류별 근거 순서로 읽으면 됩니다.</p></div>';
    var priority=all.filter(function(i){return i.severity==='confirmed';});
    if(priority.length) h+='<div class="pc-priority"><b>★ 먼저 볼 점검</b><ul>'+priority.map(function(i){return '<li><button type="button" onclick="Notices.jumpCheck(\''+esc(i.id)+'\')">'+esc(i.title)+'</button><span>'+esc(i.statusDetail||'문서상 요건 미충족 확인')+'</span></li>';}).join('')+'</ul></div>';
    ...
    CHECK_THREADS.forEach(function(th){
      ...
      shown+=items.length;
      h+='<section class="pc-thread"><header class="pc-thread-head"><div><h2>'+esc(th.t)+'</h2><p>'+items.length+'건의 점검 기록</p></div>'+flowHtml(th.flow)+'</header>'+
  ...
  return {
    /* 잠긴 탭은 **받아오지도 않는다**(&&!locked()). 화면만 가리고 fetch는 하면
       네트워크 탭에 그대로 남아 잠금이 시늉이 된다. */
