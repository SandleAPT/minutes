...
const TENANT_TERMS = {
  1:{start:"2016-03-27", end:"2018-03-26", approx:false, note:"첫 임시회의(2016.3.27)에서 임기를 2016.4.1~2018.3.31에서 2016.3.27~2018.3.26으로 변경 의결. 정원 10명 전원 구성(구자선 회장·진세택 부회장·최성인 감사·윤정희 201·김용기 202·하연서 205·김광회 206·박동선 212·김시원 215·백남신 216) → 재적 9명(2017.5)→7명(2017.9)으로 감소. 2018.2.27 회의가 마지막. 당시 선거구 10개→2017.11 규약 개정으로 9개 통합"},
  2:{start:"2018-03-27", end:"", approx:true, note:"1기 임기 만료(2018.3.26) 이튿날 취임 추정 — 선거 2018.3.8~10(후보 공고 2.27: 윤정희·진세택·옥승한·김현영·허은영·임영균 6명). 3.30 첫 소집 유회 → 5.11 임원 선출(회장 진세택·감사 한경열, 부회장 공석→7.25 허은영 격상). 재적 5→6(한경열)→7명(윤정희, 8월). 18차 임시 개최 공고(2020.3.20) 이후 결과공고 없음"},
  3:{start:"", end:"2022-05-24", approx:true,  note:"시작일 미확인(선거 공고 미게시 — 절차 점검 c_t3_election). 1차 임시 2020.06.23(재적 5명)→1차 정기 2020.08.26 임원 선출(재적 7명: 진세택 회장·구자선 부회장·원영해 감사·김영학·한경열·박동선·신명순), 2022.05.23 마지막 회의. 종료일은 4기 임기 시작(2022.05.25) 전날로 적음"},
  4:{start:"2022-05-25", end:"2024-05-24", approx:false, note:"2022.05.17~19 선거(1·2차 무입후보→3차 공고), 05.23 당선인 공고 5명(제2·4·5·8·9선거구). 임기는 보궐 선출공고(22.11.03) '당선일로부터 2024.05.24.까지'와 5기 선출공고 '4기 임기 2024.05.24 만료' 기준. 22.8월 전승환 이탈(4명)→22.11.24 보궐 강명순 합류(5명)"},
  5:{start:"2024-05-25", end:"2025-10-08", approx:false, note:"2024.05.20~22 선거(1·2차 무입후보→3차 공고), 05.23 당선인 공고 6명. 임기는 5기 선출공고(24.4.8) '2024.05.25.~2025.10.08.' 명시 — 2025.10 공고 댓글('10.8 종료')과 일치. 다만 회의는 2025.12월(통합회의)까지 5기 명의로 계속됨(절차 점검 사안), 6기 취임은 2025.12.24"},
...
function votePeopleHtml(a){
  ...
  const people=currentAttendees()
    .map(rep=>{
      ...
      // 출력물에 각자 이름 칸 위에 직접 서명한다. v419~v424의 서명 빈칸은 v425에서 뺐다(사용자, 2026-09-16).
...
async function buildDocxBlob(){
  ...
  const FONT="Malgun Gothic", INK="2F332F", LINE="9D978C", SAGE="7F927A", WIDTH=10160, BODY=24 /* v420·v427: 인쇄와 같이 0.5pt(=반포인트 1)씩 두 번 작게 */;
  ...
  const rosterWidths=[840,800,1380,2060,840,800,1380,2060];
  const rosterRows=[["동","직책","성명","서명","동","직책","성명","서명"].map((v,i)=>cell(p(v,{bold:true,size:22,alignment:AlignmentType.CENTER,after:0,line:280}),rosterWidths[i],{compact:true}))];
  for(let i=0;i<Math.max(left.length,right.length,1);i++){
    const row=[];[left[i],right[i]].forEach((rep,side)=>{
      ...
      values.forEach((v,j)=>row.push(cell(p(v,{size:22,alignment:AlignmentType.CENTER,after:0,line:280,color:rep&&!attendance[String(rep.dong)]?"888888":INK}),rosterWidths[side*4+j],{shade:"FFFFFF",compact:true})));
    });rosterRows.push(row);
  }
  const guests=(state.meeting.guests||[]).filter(g=>g.name.trim()||g.position.trim());
  ...
  const sequenceTable=sequenceIsStacked(sequenceData)
    ? table(sequenceData.map((s,i)=>[cell(p(String(i+1).padStart(2,"0"),{size:18,color:"647660",alignment:AlignmentType.CENTER,after:0}),720,{compact:true}),cell(p(s,{size:22,after:0,line:280}),9440,{compact:true})]),[720,9440])
    : table([[cell(p(sequenceData.flatMap((s,i)=>[
        run(`${String(i+1).padStart(2,"0")}  `,{size:18,color:"647660"}),
        run(s,{size:22}),
        ...(i<sequenceData.length-1?[run("     →     ",{size:20,color:"999B95"})]:[])
      ]),{alignment:AlignmentType.CENTER,after:0,line:280}),10160,{compact:true})]],[10160]);
  const children=[];
  children.push(p(docTitle(),{bold:true,size:38,alignment:AlignmentType.CENTER,after:120,line:390,border:titleBorders}));
  children.push(table([
    [cell(p("회의명",{bold:true,size:22,alignment:AlignmentType.CENTER,after:0,line:280}),1422,{compact:true}),new TableCell({children:[p(buildMeetingName(),{bold:true,size:22,after:0,line:280})],columnSpan:3,width:{size:8738,type:WidthType.DXA},verticalAlign:VerticalAlign.CENTER,margins:{top:45,bottom:45,left:90,right:90}})],
    [cell(p("일시",{bold:true,size:22,alignment:AlignmentType.CENTER,after:0,line:280}),1422,{compact:true}),cell(p(formattedMeetingDateTime(),{size:22,after:0,line:280}),3860,{compact:true}),cell(p("장소",{bold:true,size:22,alignment:AlignmentType.CENTER,after:0,line:280}),1220,{compact:true}),cell(p(state.meeting.place,{size:22,after:0,line:280}),3658,{compact:true})]
  ],[1422,3860,1220,3658]));
  ...
  children.push(sectionTitle("배석자 · 참관 현황"),table([
    [cell(p("배석자",{bold:true,size:22,alignment:AlignmentType.CENTER,after:0}),1220,{compact:true}),cell(formatted(guestText,{size:22}),5580,{compact:true}),cell(p("참관",{bold:true,size:22,alignment:AlignmentType.CENTER,after:0}),1840,{compact:true}),cell(p(audienceText,{size:22,alignment:AlignmentType.CENTER,after:0}),1520,{compact:true})]
  ],[1220,5580,1840,1520]));
  ...
  for(const item of outputAgendaItems(exportIncludeDrafts)){
    ...
    if(a.showMaterials&&printableMaterials.length){
      ...
      const rows=[["자료명","링크 · 파일명 · 보관 위치","검토 내용 · 비고"].map((v,i)=>cell(p(v,{bold:true,size:24,alignment:AlignmentType.CENTER,after:0}),[2200,3400,4560][i]))];
      printableMaterials.forEach(m=>rows.push([cell(m.title||m.fileName||"자료명 미입력",2200),cell([m.reference,m.fileName].filter(Boolean).join("\n")||" ",3400),cell(formatted(m.note,{size:24}),4560)]));
    ...
    if(voteIsBlank(a)){
      ...
      children.push(p([ // v418: 만장일치여도 집계줄·사람별 칸 모두 (서명용)
        run(`찬성 ${vote.forCount}`,{bold:true,size:23,color:"2F5128"}),
        run(`   ·   반대 ${vote.againstCount}`,{bold:true,size:23,color:"8A2A20"})
      ],{after:45,line:290}));
      const personRows=currentAttendees().map(rep=>{
        const stateName=(a.votes||{})[actorKey(rep)]||"";
        const label=stateName==="for"?"찬성":stateName==="against"?"반대":"미선택";
        const shade=stateName==="for"?"E6F0E1":stateName==="against"?"FDE9E6":"FFF4D6";
        // v425: 서명 열을 뺐다 — 이름 칸 위에 직접 서명(인쇄와 같은 방식).
        return [cell(actorFullLabel(rep),8160,{compact:true}),cell(p(label,{bold:true,size:21,alignment:AlignmentType.CENTER,after:0}),2000,{shade,compact:true})];
      });
    ...
    for(const material of includedPdfMaterials(a)){
      ...
      for(let i=0;i<images.length;i++){
        ...
        const w=Math.max(1,Math.round(dims.w*fit)),h=Math.max(1,Math.round(dims.h*fit));
        children.push(pageBreak(),p(`${item.label} · ${material.fileName} · ${i+1} / ${images.length}쪽`,{bold:true,size:16,color:"666666",after:28}),new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:0},children:[new ImageRun({data:bytes,type:"jpg",transformation:{width:w,height:h}})]}));
      }
    }
  }
  const footer=new Footer({children:[new Paragraph({alignment:AlignmentType.RIGHT,children:[run("",{size:14,color:"77766F"}),new TextRun({children:[PageNumber.CURRENT],font:FONT,size:14,color:"77766F"}),run(" / ",{size:14,color:"77766F"}),new TextRun({children:[PageNumber.TOTAL_PAGES],font:FONT,size:14,color:"77766F"})]})]});
